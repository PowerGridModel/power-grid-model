...
```{note}
When a current sensor with `global` `angle_measurement_type` is present there needs to be a voltage sensor with `u_angle_measured` in the grid to perform a state estimation. 
```
