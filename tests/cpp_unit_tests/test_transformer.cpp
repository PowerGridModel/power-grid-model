...
TEST_CASE("Test transformer") {
    ...
    SUBCASE("Update inverse") {
        ...
        SUBCASE("From status") {
            SUBCASE("same") { transformer_update.from_status = int_status(transformer.from_status()); }
            SUBCASE("different") { transformer_update.from_status = IntS{0}; }
            expected.from_status = int_status(transformer.from_status());
        }

        SUBCASE("To status") {
            SUBCASE("same") { transformer_update.to_status = int_status(transformer.to_status()); }
            SUBCASE("different") { transformer_update.to_status = IntS{0}; }
            expected.to_status = int_status(transformer.to_status());
        }
