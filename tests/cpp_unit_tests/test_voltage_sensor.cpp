...
TEST_CASE("Test voltage sensor") {
    ...
    SUBCASE("Test sym/asym calc_param for asymmetric voltage sensor, angle") {
        ...
        CHECK(real(asym_sensor_sym_param.value) == doctest::Approx((1.01 * cos(0.1) + 1.02 * cos(0.2) + 1.03 * cos(0.3)) / 3));
        CHECK(imag(asym_sensor_sym_param.value) == doctest::Approx((1.01 * sin(0.1) + 1.02 * sin(0.2) + 1.03 * sin(0.3)) / 3));
