...
TEST_CASE("Test column row conversion") {
    SUBCASE("Test meta input data generation") {
        ...
        CHECK(node_attr[1].name == "u_rated"s); // NOLINT(misc-include-cleaner)
        CHECK(node_attr[1].ctype == CType::c_double);

        auto const& sym_voltage_sensor = meta_map.get_component("sym_voltage_sensor"s); // NOLINT(misc-include-cleaner)
        auto const& sensor_attr = sym_voltage_sensor.attributes;
        CHECK(sensor_attr[0].name == "id"s); // NOLINT(misc-include-cleaner)
        CHECK(sensor_attr[0].ctype == CType::c_int32);
        CHECK(sensor_attr[1].name == "measured_object"s); // NOLINT(misc-include-cleaner)
    ...
    SUBCASE("Test meta ouput data generation") {
        ...
        CHECK(node_attr[0].name == "id"s); // NOLINT(misc-include-cleaner)
        CHECK(node_attr[0].ctype == CType::c_int32);
        CHECK(node_attr[2].name == "u_pu"s); // NOLINT(misc-include-cleaner)
        ...
        CHECK(sensor_attr[0].name == "id"s); // NOLINT(misc-include-cleaner)
        CHECK(sensor_attr[0].ctype == CType::c_int32);
        CHECK(sensor_attr[2].name == "u_residual"s); // NOLINT(misc-include-cleaner)
    ...
    SUBCASE("Test meta update data generation") {
        ...
        CHECK(load_attr[0].name == "id"s); // NOLINT(misc-include-cleaner)
        CHECK(load_attr[0].ctype == CType::c_int32);
        CHECK(load_attr[2].name == "p_specified"s); // NOLINT(misc-include-cleaner)
        ...
        CHECK(sensor_attr[0].name == "id"s); // NOLINT(misc-include-cleaner)
        CHECK(sensor_attr[0].ctype == CType::c_int32);
        CHECK(sensor_attr[1].name == "u_sigma"s); // NOLINT(misc-include-cleaner)
