...
def test_update_input_data():
    ...
    input_test[ComponentAttribute.id] = [4, 5, 6, 1, 2, 3]
    input_test[ComponentAttribute.p_specified] = [4.0, 5.0, 6.0, 1.0, 2.0, 3.0]
    input_test[ComponentAttribute.q_specified] = [4.1, 5.1, 6.1, np.nan, np.nan, np.nan]

    update_test = initialize_array(DatasetType.update, ComponentType.sym_load, 4)
    update_test[ComponentAttribute.id] = [6, 5, 2, 3]
    update_test[ComponentAttribute.q_specified] = [np.nan, 5.2, np.nan, 3.2]

    merged = _update_input_data(
        input_data={ComponentType.sym_load: input_test}, update_data={ComponentType.sym_load: update_test}
    )
    np.testing.assert_array_equal(merged[ComponentType.sym_load][ComponentAttribute.id], [4, 5, 6, 1, 2, 3])
    np.testing.assert_array_equal(
        merged[ComponentType.sym_load][ComponentAttribute.p_specified], [4.0, 5.0, 6.0, 1.0, 2.0, 3.0]
    )
    np.testing.assert_array_equal(
        merged[ComponentType.sym_load][ComponentAttribute.q_specified], [4.1, 5.2, 6.1, np.nan, np.nan, 3.2]
    )
...
def test_update_input_data__without_ids():
    ...
    input_test[ComponentAttribute.q_specified] = [4.1, 5.2, np.nan, np.nan, np.nan, 3.2]
    ...
    np.testing.assert_array_equal(
        merged[ComponentType.sym_load][ComponentAttribute.q_specified], [4.1, 5.2, np.nan, np.nan, np.nan, 3.2]
    )


def test_update_input_data_int_nan():
    input_line = initialize_array(DatasetType.input, ComponentType.line, 3)
    input_line[ComponentAttribute.id] = [1, 2, 3]
    input_line[ComponentAttribute.from_status] = [0, -128, -128]

    update_line = initialize_array(DatasetType.update, ComponentType.line, 2)
    update_line[ComponentAttribute.id] = [1, 3]
    update_line[ComponentAttribute.from_status] = [-128, 1]

    merged = _update_input_data(
        input_data={ComponentType.line: input_line}, update_data={ComponentType.line: update_line}
    )

    np.testing.assert_array_equal(merged[ComponentType.line][ComponentAttribute.from_status], [0, -128, 1])


def test_update_input_data_asym_nans():
    input_load = initialize_array(DatasetType.input, ComponentType.asym_load, 3)
    input_load[ComponentAttribute.id] = [1, 2, 3]
    input_load[ComponentAttribute.p_specified] = [[1.1, 1.2, 1.3], [2.1, np.nan, np.nan], [np.nan, np.nan, np.nan]]

    update_load = initialize_array(DatasetType.update, ComponentType.asym_load, 3)
    update_load[ComponentAttribute.id] = [1, 2, 3]
    update_load[ComponentAttribute.p_specified] = [[np.nan, np.nan, np.nan], [np.nan, np.nan, 5.3], [6.1, 6.2, 6.3]]
    ...
    np.testing.assert_array_equal(
        merged[ComponentType.asym_load][ComponentAttribute.p_specified],
...
def test_errors_to_string_list_1():
    errors = [NotGreaterThanError(ComponentType.node, "u_rated", [1], 0)]
