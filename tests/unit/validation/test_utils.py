...
def test_update_input_data():
    input_test = initialize_array(DatasetType.input, CT.sym_load, 6)
    input_test[AT.id] = [4, 5, 6, 1, 2, 3]
    input_test[AT.p_specified] = [4.0, 5.0, 6.0, 1.0, 2.0, 3.0]
    input_test[AT.q_specified] = [4.1, 5.1, 6.1, np.nan, np.nan, np.nan]

    update_test = initialize_array(DatasetType.update, CT.sym_load, 4)
    update_test[AT.id] = [6, 5, 2, 3]
    update_test[AT.q_specified] = [np.nan, 5.2, np.nan, 3.2]

    merged = _update_input_data(input_data={CT.sym_load: input_test}, update_data={CT.sym_load: update_test})
    np.testing.assert_array_equal(merged[CT.sym_load][AT.id], [4, 5, 6, 1, 2, 3])
    np.testing.assert_array_equal(merged[CT.sym_load][AT.p_specified], [4.0, 5.0, 6.0, 1.0, 2.0, 3.0])
    np.testing.assert_array_equal(merged[CT.sym_load][AT.q_specified], [4.1, 5.2, 6.1, np.nan, np.nan, 3.2])
...
def test_update_input_data__without_ids():
    ...
    update_test = initialize_array(DatasetType.update, CT.sym_load, 6)
    input_test[AT.q_specified] = [4.1, 5.2, np.nan, np.nan, np.nan, 3.2]

    merged = _update_input_data(input_data={CT.sym_load: input_test}, update_data={CT.sym_load: update_test})
    np.testing.assert_array_equal(merged[CT.sym_load][AT.id], [4, 5, 6, 1, 2, 3])
    np.testing.assert_array_equal(merged[CT.sym_load][AT.p_specified], [4.0, 5.0, 6.0, 1.0, 2.0, 3.0])
    np.testing.assert_array_equal(merged[CT.sym_load][AT.q_specified], [4.1, 5.2, np.nan, np.nan, np.nan, 3.2])


def test_update_input_data_int_nan():
    input_line = initialize_array(DatasetType.input, CT.line, 3)
    input_line[AT.id] = [1, 2, 3]
    input_line[AT.from_status] = [0, -128, -128]

    update_line = initialize_array(DatasetType.update, CT.line, 2)
    update_line[AT.id] = [1, 3]
    update_line[AT.from_status] = [-128, 1]

    merged = _update_input_data(input_data={CT.line: input_line}, update_data={CT.line: update_line})

    np.testing.assert_array_equal(merged[CT.line][AT.from_status], [0, -128, 1])


def test_update_input_data_asym_nans():
    input_load = initialize_array(DatasetType.input, CT.asym_load, 3)
    input_load[AT.id] = [1, 2, 3]
    input_load[AT.p_specified] = [[1.1, 1.2, 1.3], [2.1, np.nan, np.nan], [np.nan, np.nan, np.nan]]

    update_load = initialize_array(DatasetType.update, CT.asym_load, 3)
    update_load[AT.id] = [1, 2, 3]
    update_load[AT.p_specified] = [[np.nan, np.nan, np.nan], [np.nan, np.nan, 5.3], [6.1, 6.2, 6.3]]

    merged = _update_input_data(input_data={CT.asym_load: input_load}, update_data={CT.asym_load: update_load})

    np.testing.assert_array_equal(
        merged[CT.asym_load][AT.p_specified],
...
def test_errors_to_string_list_1():
    errors = [NotGreaterThanError(CT.node, AT.u_rated, [1], 0)]
...
def test_errors_to_string_list_2_no_details():
    errors = [
        NotGreaterThanError(CT.node, AT.u_rated, [1], 0),
        NotGreaterThanError(CT.line, AT.p_from, [2], 0),
    ]
    error_string = errors_to_string(errors=errors, name="input_data")
    assert (
...
def test_errors_to_string_dict_2_no_details():
    errors = {
        1: [NotGreaterThanError(CT.node, AT.u_rated, [1], 0)],
        2: [NotGreaterThanError(CT.line, AT.p_from, [2], 0)],
    }
    error_string = errors_to_string(errors=errors, name="input_data")
    assert (
