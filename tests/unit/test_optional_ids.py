...
def input_data_r():
    ...
    node[AT.id] = np.array([1, 2, 6])
    node[AT.u_rated] = [10.5e3, 10.5e3, 10.5e3]

    line = initialize_array(DatasetType.input, CT.line, 3)
    line[AT.id] = [3, 5, 8]
    line[AT.from_node] = [1, 2, 1]
    line[AT.to_node] = [2, 6, 6]
    line[AT.from_status] = [1, 1, 1]
    line[AT.to_status] = [1, 1, 1]
    line[AT.r1] = [0.25, 0.25, 0.25]
    line[AT.x1] = [0.2, 0.2, 0.2]
    line[AT.c1] = [10e-6, 10e-6, 10e-6]
    line[AT.tan1] = [0.0, 0.0, 0.0]
    line[AT.i_n] = [1000, 1000, 1000]

    sym_load = initialize_array(DatasetType.input, CT.sym_load, 2)
    sym_load[AT.id] = [4, 7]
    sym_load[AT.node] = [2, 6]
    sym_load[AT.status] = [1, 1]
    sym_load[AT.type] = [LoadGenType.const_power, LoadGenType.const_power]
    sym_load[AT.p_specified] = [20e6, 10e6]
    sym_load[AT.q_specified] = [5e6, 2e6]
...
def update_sym_load_no_id_r():
    sym_load = initialize_array(DatasetType.update, CT.sym_load, (2, 2))
    sym_load[AT.p_specified] = [[30e6, 10e6], [30e6, 15e6]]
    return sym_load


@pytest.fixture
def update_sym_load_c():
    source_attribute_dtypes = power_grid_meta_data[DatasetType.update][CT.sym_load].dtype
    return {
        AT.id: np.array([[4], [7]], dtype=source_attribute_dtypes[AT.id]),
        AT.p_specified: np.array([[30e6], [15e6]], dtype=source_attribute_dtypes[AT.p_specified]),
    }
...
def update_sym_load_no_id_c():
    source_attribute_dtypes = power_grid_meta_data[DatasetType.update][CT.sym_load].dtype
    return {
        AT.p_specified: np.array([[30e6, 10e6], [30e6, 15e6]], dtype=source_attribute_dtypes[AT.p_specified]),
    }
...
@pytest.fixture
def update_line_r():
    line = initialize_array(DatasetType.update, CT.line, (2, 1))
    line[AT.id] = [[3], [5]]
    line[AT.from_status] = [[0], [0]]
    return line


@pytest.fixture
def update_line_no_id_r():
    line = initialize_array(DatasetType.update, CT.line, (2, 3))
    line[AT.from_status] = [[0, 1, 1], [0, 0, 1]]
    return line


@pytest.fixture
def update_line_c():
    source_attribute_dtypes = power_grid_meta_data[DatasetType.update][CT.line].dtype
    return {
        AT.id: np.array([[3], [5]], dtype=source_attribute_dtypes[AT.id]),
        AT.from_status: np.array([[0], [0]], dtype=source_attribute_dtypes[AT.from_status]),
    }
...
def update_line_no_id_c():
    source_attribute_dtypes = power_grid_meta_data[DatasetType.update][CT.line].dtype
    return {
        AT.from_status: np.array([[0, 1, 1], [0, 0, 1]], dtype=source_attribute_dtypes[AT.from_status]),
    }
