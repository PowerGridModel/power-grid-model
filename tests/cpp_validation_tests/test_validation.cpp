    ...
    static bool match_exception(std::exception const& e, std::string_view message) {
        ...
        std::map<std::string, std::regex, std::less<>> const error_mapping = {
            ...
            {"UnsupportedRegulatorCombinationError",
             std::regex{"The combination of voltage regulators and transformer tap regulators is not supported in the same model."}},
            {"UnsupportedVoltageRegulatorSourceCombinationError",
             std::regex{"Nodes with a source and a voltage regulated load/generator are not supported when both are enabled. Found at node with id (-?\\d+)"}},
