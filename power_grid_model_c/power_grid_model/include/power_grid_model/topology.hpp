  ...
  public:
    ...
    build_topology() {
        ...
        std::pair<std::vector<std::shared_ptr<MathModelTopology const>>,
                  std::shared_ptr<TopologicalComponentToMathCoupling const>>
            pair;
        for (Idx const k : IdxRange{std::ssize(math_topology_)}) {
  ...
  private:
    ...
    void dfs_search() {
        ...
        for (auto const& [source_connected, source_node] :
            ...
            ++math_solver_idx;
            assert(math_solver_idx == std::ssize(math_topology_));
        }
    }
    ...
    std::vector<BranchIdx> reorder_node(std::vector<Idx>& dfs_node,
        ...
        for (Idx const idx : IdxRange{std::ssize(reordered)}) {
    ...
    void couple_branch() {
        ...
        for (auto&& [idx, branch_node_idx, branch_connected] :
            ...
            assert(std::ssize(branch_connected) == 2); // NOSONAR(R354) // NOLINT(cert-dcl03-c,hicpp-static-assert,misc-static-assert)
        ...
        for (auto&& [idx, i, i_status, j_math] :
            ...
            // set branch idx in coupling
            comp_coup_.branch3[idx] = idx_branch3;
        }
    }
    ...
    void couple_object_components(GetMathTopoComponent get_component_topo, ObjectFinder object_finder,
        ...
        for (Idx topo_idx = 0; topo_idx != n_math_topologies; ++topo_idx) {
            ...
            for (auto const [new_math_comp_i, old_math_comp_i] :
                 std::views::zip(IdxRange(std::ssize(reorder)), reorder)) {
