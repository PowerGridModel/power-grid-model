...
template <scalar_value T> class Tensor : public Eigen3Tensor<T> {
  public:
    ...
    explicit Tensor(Vector<T> const& v) { static_assert(v.size() == 3); (*this) << v(0), 0.0, 0.0, 0.0, v(1), 0.0, 0.0, 0.0, v(2); }
    // eigen expression
...
template <scalar_value T> class Tensor4 : public Eigen4Tensor<T> {
  public:
    ...
    explicit Tensor4(Vector<T> const& v) { static_assert(v.size() == 4); (*this) << v(0), 0.0, 0.0, 0.0, 0.0, v(1), 0.0, 0.0, 0.0, 0.0, v(2), 0.0, 0.0, 0.0, 0.0, v(3); }
    // eigen expression
