...
class Edge : public Base {
  public:
    ...
    explicit Edge(BranchInput const& edge_input)
        : Base{edge_input},
          from_node_{edge_input.from_node},
          to_node_{edge_input.to_node},
          from_status_{static_cast<bool>(edge_input.from_status)},
          to_status_{static_cast<bool>(edge_input.to_status)} {}
    ...
    constexpr bool edge_status() const { return from_status() && to_status(); }
    ...
    template <symmetry_tag sym> BranchCalcParam<sym> calc_param(bool is_connected_to_source = true) const {
        // TODO(mgovers): cleanup v2: move to Branch
    ...
    BranchOutput<sym> get_output(BranchSolverOutput<sym> const& edge_solver_output) const {
        ...
        // calculate result
        output.p_from = base_power<sym> * real(edge_solver_output.s_f);
        output.q_from = base_power<sym> * imag(edge_solver_output.s_f);
        output.i_from = base_i_from() * cabs(edge_solver_output.i_f);
        output.s_from = base_power<sym> * cabs(edge_solver_output.s_f);
        output.p_to = base_power<sym> * real(edge_solver_output.s_t);
        output.q_to = base_power<sym> * imag(edge_solver_output.s_t);
        output.i_to = base_i_to() * cabs(edge_solver_output.i_t);
        output.s_to = base_power<sym> * cabs(edge_solver_output.s_t);
    ...
    get_sc_output(BranchShortCircuitSolverOutput<asymmetric_t> const& edge_solver_output) const {
        BranchShortCircuitOutput output{};
        static_cast<BaseOutput&>(output) = base_output(true);
        // calculate result
        output.i_from = base_i_from() * cabs(edge_solver_output.i_f);
        output.i_to = base_i_to() * cabs(edge_solver_output.i_t);
        output.i_from_angle = arg(edge_solver_output.i_f);
        output.i_to_angle = arg(edge_solver_output.i_t);
        return output;
    }

    BranchShortCircuitOutput
    get_sc_output(BranchShortCircuitSolverOutput<symmetric_t> const& edge_solver_output) const {
        return get_sc_output(
            BranchShortCircuitSolverOutput<asymmetric_t>{.i_f = ComplexValue<asymmetric_t>{edge_solver_output.i_f},
                                                         .i_t = ComplexValue<asymmetric_t>{edge_solver_output.i_t}});
    }
    ...
    // default update for edge, will be hidden for transformer
    UpdateChange update(BranchUpdate const& update_data) {
        ...
        // change edge connection will change both topo and param
  ...
  protected:
    // calculate edge param based on symmetric component
    BranchCalcParam<symmetric_t>
    calc_param_y_sym(DoubleComplex const& y_series, // y_series must be converted to the "to" side of the edge
                     DoubleComplex const& y_shunt,  // y_shunt must be converted to the "to" side of the edge
        ...
        if (!edge_status()) {
            ...
            if (from_status() || to_status()) {
                DoubleComplex edge_shunt;
                // shunt value
                if (cabs(y_shunt) < numerical_tolerance) {
                    edge_shunt = 0.0;
                } else {
                    // edge_shunt = y_shunt/2 + 1/(1/y_series + 2/y_shunt) // NOSONAR(S125)
                    edge_shunt = 0.5 * y_shunt + 1.0 / (1.0 / y_series + 2.0 / y_shunt);
                }
                // from or to connected
                param.yff() = from_status() ? (1.0 / tap / tap) * edge_shunt : 0.0;
                param.ytt() = to_status() ? edge_shunt : 0.0;
            }
        }
        // both connected
        else {
    ...
    // calculate edge param for asymmetric
