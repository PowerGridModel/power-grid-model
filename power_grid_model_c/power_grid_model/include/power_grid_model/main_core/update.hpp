...
struct UpdateCompProperties {
    ...
    Idx elements_ps_in_update{invalid_index_v}; // count of elements for this component per scenario in update
    Idx elements_in_base{invalid_index_v};      // count of elements for this component per scenario in input
    ...
    constexpr Idx get_n_elements() const {
        assert(uniform || elements_ps_in_update == invalid_index_v);
...
UpdateCompProperties check_component_independence(ConstDataset const& update_data, Idx n_component) {
    ...
    properties.has_any_elements =
        component_idx != invalid_index_v && update_data.get_component_info(component_idx).total_elements > 0;
    properties.elements_ps_in_update =
        properties.uniform ? update_data.uniform_elements_per_scenario(properties.name) : invalid_index_v;
...
utils::ComponentFlags<ComponentTypes...> is_update_independent(ConstDataset const& update_data,
                                                               std::span<Idx const> relevant_component_count) {
    utils::ComponentFlags<ComponentTypes...> result{};
    size_t comp_idx{};
...
template <class... ComponentTypes, class ComponentContainer>
utils::SequenceIdx<ComponentTypes...> get_all_sequence_idx_map(MainModelState<ComponentContainer> const& state,
                                                               ConstDataset const& update_data) {
    utils::ComponentFlags<ComponentTypes...> components_to_store{};
    Idx comp_idx{};
    utils::run_functor_with_all_types_return_void<ComponentTypes...>(
        [&update_data, &components_to_store, &comp_idx]<typename CompType>() {
            components_to_store[comp_idx] = (update_data.find_component(CompType::name, false) != invalid_index_v);
            ++comp_idx;
        });
