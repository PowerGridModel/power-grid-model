...
inline void add_component(MainModelState<ComponentContainer>& state, Inputs&& component_inputs,
    ...
    reserve_component<Component>(state.components, std::ranges::size(component_inputs));
    ...
    for (auto const& input_proxy : std::views::all(std::forward<Inputs>(component_inputs))) {
        ...
        if constexpr (std::derived_from<Component, Node>) {
            emplace_component<Component>(state.components, id, input);
        } else if constexpr (std::derived_from<Component, Branch>) {
            double const u1 = get_component<Node>(state.components, input.from_node).u_rated();
            double const u2 = get_component<Node>(state.components, input.to_node).u_rated();
            // set system frequency for line
            if constexpr (std::same_as<Component, Line> || std::same_as<Component, AsymLine>) {
                emplace_component<Component>(state.components, id, input, system_frequency, u1, u2);
            } else {
                emplace_component<Component>(state.components, id, input, u1, u2);
            }
        } else if constexpr (std::derived_from<Component, Branch3>) {
            double const u1 = get_component<Node>(state.components, input.node_1).u_rated();
            double const u2 = get_component<Node>(state.components, input.node_2).u_rated();
            double const u3 = get_component<Node>(state.components, input.node_3).u_rated();
            emplace_component<Component>(state.components, id, input, u1, u2, u3);
        } else if constexpr (std::derived_from<Component, Appliance>) {
            double const u = get_component<Node>(state.components, input.node).u_rated();
            emplace_component<Component>(state.components, id, input, u);
        } else if constexpr (std::derived_from<Component, GenericVoltageSensor>) {
            double const u = get_component<Node>(state.components, input.measured_object).u_rated();
            emplace_component<Component>(state.components, id, input, u);
        } else if constexpr (std::derived_from<Component, GenericPowerSensor>) {
            // it is not allowed to place a sensor at a link
            if (get_component_idx_by_id(state.components, input.measured_object).group ==
                get_component_type_index<Link>(state.components)) {
            ...
            case branch_from:
                [[fallthrough]];
            case branch_to:
                get_component<Branch>(state.components, measured_object);
                break;
            case branch3_1:
                [[fallthrough]];
            case branch3_2:
                [[fallthrough]];
            case branch3_3:
                get_component<Branch3>(state.components, measured_object);
                break;
            case shunt:
                get_component<Shunt>(state.components, measured_object);
                break;
            case source:
                get_component<Source>(state.components, measured_object);
                break;
            case load:
                get_component<GenericLoad>(state.components, measured_object);
                break;
            case generator:
                get_component<GenericGenerator>(state.components, measured_object);
                break;
            case node:
                get_component<Node>(state.components, measured_object);
                break;
            default:
            ...
            ID const node = [&state, measured_object = input.measured_object,
                ...
                case branch_from:
                    return get_component<Branch>(state.components, measured_object).node(BranchSide::from);
                case branch_to:
                    return get_component<Branch>(state.components, measured_object).node(BranchSide::to);
                case branch3_1:
                    return get_component<Branch3>(state.components, measured_object).node(side_1);
                case branch3_2:
                    return get_component<Branch3>(state.components, measured_object).node(side_2);
                case branch3_3:
                    return get_component<Branch3>(state.components, measured_object).node(side_3);
                default:
            ...
            double const u_rated = get_component<Node>(state.components, node).u_rated();

            emplace_component<Component>(state.components, id, input, u_rated);
            ...
            get_component<Node>(state.components, input.fault_object);
            emplace_component<Component>(state.components, id, input);
        } else if constexpr (std::derived_from<Component, TransformerTapRegulator>) {
            Idx2D const regulated_object_idx =
                main_core::get_component_idx_by_id<ComponentContainer>(state.components, input.regulated_object);
            ...
            ID const regulated_terminal = [&input, &state, &regulated_object_idx] {
                ...
                if (regulated_object_idx.group == get_component_type_index<Transformer>(state.components)) {
                    auto const& regulated_object = get_component<Transformer>(state.components, regulated_object_idx);
                    ...
                    default:
                        throw MissingCaseForEnumError{std::format("{} item retrieval", Component::name),
                           ...
                           get_component_type_index<ThreeWindingTransformer>(state.components)) {
                    auto const& regulated_object =
                        get_component<ThreeWindingTransformer>(state.components, regulated_object_idx);
                    switch (input.control_side) {
                    case side_1:
                        [[fallthrough]];
                    case side_2:
            ...
            if (regulated_object_idx.group != get_component_type_index<Transformer>(state.components) &&
                regulated_object_idx.group != get_component_type_index<ThreeWindingTransformer>(state.components)) {
                throw InvalidRegulatedObject(input.regulated_object, Component::name);
            }

            auto const regulated_object_type =
                get_component<Base>(state.components, regulated_object_idx).math_model_type();
            double const u_rated = get_component<Node>(state.components, regulated_terminal).u_rated();

            emplace_component<Component>(state.components, id, input, regulated_object_type, u_rated);
        }
    }
