    ...
    call_with_catch(
        ...
        [serializer, use_compact_list, data, size] {
            auto const buffer_data =
                safe_ptr_get(cast_to_cpp(serializer)).get_binary_buffer(safe_bool(use_compact_list));
...
char const* PGM_serializer_get_to_zero_terminated_string(PGM_Handle* handle, PGM_Serializer* serializer,
    ...
    return call_with_catch(
        ...
        [serializer, use_compact_list, indent] {
            return safe_ptr_get(cast_to_cpp(serializer)).get_string(safe_bool(use_compact_list), indent).c_str();
        },
        serialization_exception_handler);
}
